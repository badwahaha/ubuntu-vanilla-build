...
- Desktop choice:
  ...
  - `minimal` — target base is an explicit no-snap approximation of **`ubuntu-server-minimal`**: the available server/storage/cloud packages are installed directly, while the metapackage and its `snapd` dependency are omitted. The live ISO ships a tiny X stack (`xserver-xorg` + `xserver-xorg-input-all` + `xserver-xorg-video-all`, `xinit`, `x11-xserver-utils`, `openbox`, `lightdm` + `lightdm-gtk-greeter`, `accountsservice`, `dbus-x11`, `network-manager-gnome`, `xterm`, `fonts-dejavu-core`) so Calamares can render. LightDM auto-logs the live `ubuntu` user into Openbox; `/etc/xdg/openbox/autostart` runs `nm-applet` and `sudo -E calamares -d`. After install, Calamares' `packages` module removes the X stack and `gparted` from the target; `settings-minimal.conf` drops the `displaymanager` exec step. Brave and Flatpak are skipped for this variant.
