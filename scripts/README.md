...
- **Repositories**: everything is served from the CDN-backed `https://apt.pop-os.org/` — the `ubuntu` mirror for debootstrap/base packages, plus the `release`, `proprietary`, and `release-ubuntu` suites configured inside the chroot (`apt-origin.pop-os.org` is only a per-suite fallback when the CDN does not publish a suite). All are signed with the Pop!_OS archive keyring (`pop-keyring` takes over after bootstrap) and pinned at priority 1001 (`o=pop-os-release`) so Pop!_OS packages win over the Ubuntu archive. **Staging suites are deliberately excluded.**
